#include <iostream>
#include "prepMess.h"
#include <vector>
#include "Client.h"
#define DEBUG_STEP_2

PrevMess::PrevMess()//конструктор по умолчанию
{
	managerInterLogPass = true;
}


	std::string PrevMess::addLogPass()
	{
		std::cout << " ¬ведите логин нового пользователя: " << std::endl;
		std::cout << ">> ";
		std::cin >> login;//¬водим новый логин
		std::cout << "¬ведите нового пароль: " << std::endl;
		std::cout << ">> ";
		std::cin >> password;
		std::string resultStr = password + ":" + login;
		return resultStr;
	}

	std::string PrevMess::InterfaceLogPass(bool StartFunction)//—оздаесм стартовое окно дл¤ создани¤ логина и парол¤ или авторизаци¤ на сервере
	{
			std::string tempStr;
			std::string tempOP;
		//----------------------------------------------------------------------
			if (StartFunction)
			{
				while(true)
				{ 
				std::cout << "Для продолжения нажмите любую клавишу\n ";
				std::cout << ">> ";
				std::cin >> tempOP;
				if (tempOP.size()!= 0)
						tempStr = addLogPass();
						tempStr = "*" + tempStr + ":--:2:1:1:1:--&";
						return tempStr;
				}
			}
			//----------------------------------------------------------------------

	}

	//-----------------------------------------------------------------------------------------------
	// ----------------------------Функция  авторизациии---------------------------------------------
	//---------------------------------------------------------------------------------------------
	std::string PrevMess::enterLogPass(std::string _stateProgram)
	{
		std::string _password;
		std::string _login;
		std::string  resultStr;
		std::cout << " ¬ведите логин нового пользователя: " << std::endl;
		std::cout << ">> ";
		std::cin >> _login;//¬водим новый логин
		std::cout << "¬ведите пароль\n";
		std::cout << ">> ";
		std::cin >> _password;

#ifdef  DEBUG_STEP_2
		_stateProgram = "7";
#endif
		resultStr = "*" + _password + ":" + _login + ":--:6:" + _stateProgram + ":1:1:--&";
		return resultStr;
	}
	void  PrevMess::showMessages()
	{
		int a = 5;
	}